...
$('#find_doctors_enterbox').keyup(function(event){if(event.keyCode == 37 || event.keyCode == 38 || event.keyCode == 39 || event.keyCode == 40){return false;}
var s=this,qword=$(s).val();if($.trim(qword)==''){$('.SmartSearchBox').hide();$(s).val('').focus();return false;}
if($.trim(qword)!= ''){$('.SmartSearchBox').show();$(".SmartSearchBox").html('<div align="center" style="padding:8px;"><i class="fa fa-spinner fa-spin" style="color:#FF3300;font-size:25px;"></i></div>');$('body').keyup(function(event){if(event.keyCode == 27){
$('.SmartSearchBox').hide();return false;}});$.ajax({type: "POST",url: turl+"doctors/", data:{rs:true,search_doc:qword,search_entry:true}, success: function(html){
$(".SmartSearchBox").html(html);},error: function(html){$(".SmartSearchBox").html('<div style="text-align:center;font-weight:bold;">'+err_conn+'</div>');}});}});
...
$(".LoadAppointments_ajax").trigger("click");
...
$(".BookAppointment_ajax").live("click", function(e){e.preventDefault();var t=this,did=$(t).attr("id");
$("#BookAppointment"+did).hide();
$(t).addClass("opacity");
$.ajax({type:"POST",url:turl+"appointment/",timeout:15000,data:{calendar:true,cal_uid:did},cache:false,success:function(html){
$("#BookAppointment"+did).show().html(html);
$(t).removeClass("opacity");
},error:function(html){$(t).removeClass("opacity");}});});
...
$("#Appointment"+did).html('<div style="padding:10px;background-color:#999999;color:#000;font-size:16px;font-weight:bold;margin-top:10px;">'+dateA+'<br><br>Are you sure want to appointment this doctor? <div class="docBut AppointmentActive_ajax" date="'+dateA+'" did="'+did+'" >Yes, Appointment</div></div>');										  
});
...
$(".show_search_ajax").live("click", function(e){e.preventDefault();var t=this;
$(t).addClass("activeMenu");	
$(".show_category_ajax").removeClass("activeMenu");	
$(".show_search_box_ajax").removeClass("activeMenu");										

$(".SearchBox").hide();
$(".SearchEnterBox").hide();
$(".CategoryBox").show();
$("#LoadDoctors").show().html('');
});
$(".show_category_ajax").live("click", function(e){e.preventDefault();var t=this;
$(t).addClass("activeMenu");	
$(".show_search_box_ajax").removeClass("activeMenu");										
$(".show_search_ajax").removeClass("activeMenu");										
$(".CategoryBox").hide();
$(".SearchBox").show();
$("#LoadDoctors").show().html('');
});

$(".show_search_box_ajax").live("click", function(e){e.preventDefault();var t=this;
$(t).addClass("activeMenu");	
$(".show_category_ajax").removeClass("activeMenu");										
$(".show_search_ajax").removeClass("activeMenu");										
$(".CategoryBox,.SearchBox").hide();
$(".SearchEnterBox").show();
$("#find_doctors_enterbox").focus();
...
$("#stype").live("change", function(e){e.preventDefault();var t=this;
if("Patient"==$(t).val())$(".sage").show();else $(".sage").hide();
});
...
$(t).addClass("opacity");
$.ajax({type:"POST",url:turl+"signup/",timeout:15000,data:{sfname:sfname,slname:slname,semail:semail,spwd:spwd,stype:stype,phnumb:phnumb,age:age},cache:false,success:function(html){
$(t).removeClass("opacity");
