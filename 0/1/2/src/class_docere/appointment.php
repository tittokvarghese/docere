 ...
 */
	...
	$check_appointments = dbTKV::SQLQUERY("SELECT A.*,U.* FROM appointments A,users U  WHERE  
	
	A.apts_pid='".$_SESSION['USER_ID']."' AND A.apts_did='".$doctor_uid."' AND A.apts_seen='0'
	
	 AND U.uact='1' AND U.uid=A.apts_did 
	
	 "); 
	if(dbTKV::SQLNUM_ROWS($check_appointments))
	{
		dbTKV::SQLQUERY("DELETE FROM  appointments WHERE 
				apts_pid='".$_SESSION['USER_ID']."'	AND 	apts_did='".$doctor_uid."' AND 	apts_seen='0'	");
	}
	
	
	
		dbTKV::SQLQUERY("INSERT INTO appointments (apts_pid,apts_did,apts_day,apts_month,apts_month_num,apts_year,apts_week)
			VALUES('".$_SESSION['USER_ID']."','".$doctor_uid."','".$arr[0]."','".$arr[1]."','".MainFunc::get_month_name($arr[1])."','".$arr[2]."','".$arr[3]."') ");
		echo SITE_URL.'profiles/?id='.$doctor_uid.'&token='.$doctor_uid.'&day='.$arr[0].'&month='.$arr[1].'&year='.$arr[2].'';
...
if($type=="latest")
	$q	= "SELECT A.*,U.* FROM appointments A,users U  WHERE  A.apts_pid='".$_SESSION['USER_ID']."'	 AND U.uid=A.apts_did AND  A.apts_seen='0'	  AND U.uact='1' ORDER BY A.aptsid DESC	";
else if($type=="today")
	...
	$q	= "SELECT A.*,U.* FROM appointments A,users U  WHERE  A.apts_pid='".$_SESSION['USER_ID']."'	 AND U.uid=A.apts_did AND  A.apts_seen='1'   AND U.uact='1' GROUP BY A.apts_did ORDER BY A.aptsid DESC	";
...
echo '<div class="docBox" id="'.$row['uid'].'"  did="'.$row['aptsid'].'" ><div class="docImg"><a href="'.SITE_URL.'profiles/?id='.$row['uid'].'"><img src="'.MainFunc::GetProfilePic($row['uid']).'" class="docW" alt="..." align="absmiddle"></a></div><div class="docDiv"><div class="docTitle"><span>'.MainFunc::GetName($row['uid'],3).'</span></div>
...
<div class="docBut BookAppointment_ajax" id="<?php echo $row['uid'];?>" >Change Date</div>
<div class="docBut CancelAppointments_ajax">Cancel</div>

<div style="border:solid 0px #999999;padding:0px;margin-top:10px;display:none;" id="BookAppointment<?php echo $row['uid'];?>"></div>
