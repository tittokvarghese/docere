...
public static function Show(){
	...
	$q	= "SELECT N.*,U.* FROM notifications N,users U  WHERE  N.not_tid='".$_SESSION['USER_ID']."'	 AND U.uid=N.not_fid  AND U.uact='1'   ORDER BY N.notid DESC	";
...
echo '<div class="docBox" id="'.$row['uid'].'"  did="'.$row['notid'].'" ><div class="docImg"><a href="'.SITE_URL.(MainFunc::CheckUserDoctor($row['not_fid']) ? 'profiles' : 'patient').'/?id='.$row['uid'].'"><img src="'.MainFunc::GetProfilePic($row['uid']).'" class="docW" alt="..." align="absmiddle"></a></div><div class="docDiv"><div class="docTitle"><span>'.(MainFunc::CheckUserDoctor($row['uid']) ? 'Dr.'.MainFunc::GetName($row['uid'],3).'' : MainFunc::GetName($row['uid'],3)).' </span>';
...
else if($row['not_type']==4)
{
echo '<div style="padding:10px 0px 0px 0px;font-size:14px;color:#006633;line-height:24px;">
<i>New question received!</i> <br />
'.MainFunc::timeS($row['not_time'],0).'
</div>';
}
else if($row['not_type']==5)
{
echo '<div style="padding:10px 0px 0px 0px;font-size:14px;color:#006633;line-height:24px;">
<i>Replied your question!</i> <br />
'.MainFunc::timeS($row['not_time'],0).'
</div>';
}
