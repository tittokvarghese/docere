 ...
 */
	...
	{
		$row  = dbTKV::SQL_FETCH($time);
	
	
	
	$titto=1;
		foreach(explode(",",$row['tmt_week']) as $week)
		{
		
		
			$tmt_week	= dbTKV::SQLQUERY("SELECT * FROM appointments  WHERE  apts_did='".$_SESSION['USER_ID']."' AND apts_week='".$week."' AND apts_seen='0' "); 
			if(dbTKV::SQLNUM_ROWS($tmt_week))
			{
			echo "You cannot delete this time because some appointment has appeared in this time.";
			$titto =0;
			break;
			}
...
public static function SaveTimeTable(){
	...
	$doc_limit 		= MainFunc::Encrypt(MainFunc::filter($_POST['doc_limit']));
		...
		if(!is_numeric($doc_limit))
		{
			return "Limit should be number";
		}
		else if(!preg_match('/^[0-9\'.]+$/i',$doc_limit))
		{
			return "Limit should be number";
		}
		else if($doc_limit==0)
		...
		else if($f_hh==$t_hh && $f_time==$t_time)
		{
		...
		$doctors 	= dbTKV::SQLQUERY("SELECT * FROM timetable  WHERE  
					tmt_uid='".$_SESSION['USER_ID']."' AND tmt_week LIKE  '%".$doc_week."%'  	"); //AND tmt_ftime='".$f_time."'		
...
public static function Save(){
	...
	$mobile_number 		= MainFunc::Encrypt(MainFunc::filter($_POST['mobile_number']));
	$age_data 			= MainFunc::Encrypt(MainFunc::filter($_POST['age_data']));
	...
	$doc_upload_image 	= @$_FILES['doc_upload_image'];
		...
		else if(is_numeric($doc_fname) or is_numeric($doc_lname))
		{
			return "Name should be character";
		}

		else if(!preg_match('/^[A-Za-z \'.]+$/i',$doc_fname) or !preg_match('/^[A-Za-z \'.]+$/i',$doc_lname))
		{
			return "Name should be character";
		}
	...
	if(empty($mobile_number) or strlen($mobile_number)<1 or !is_numeric($mobile_number))
	{
	...
	if(strlen($mobile_number)>9)
		dbTKV::SQLQUERY("UPDATE users SET uphone='".$mobile_number."'				WHERE  uid='".$_SESSION['USER_ID']."' ");
	}
	...
	if($patient==1)
	{
		if(empty($age_data) or strlen($age_data)<2 or !is_numeric($age_data))
		{
		...
		else if($age_data<1)
		{
			...
			dbTKV::SQLQUERY("UPDATE users SET uage='".$age_data."'			WHERE  uid='".$_SESSION['USER_ID']."' ");	
	}	
		...
		if(!preg_match('/^[A-Za-z, \'.]+$/i',$doc_degree))
		{
			return "Degree should be character";
		}
		if(!is_numeric($doc_fee))
		{
			$doc_fee 	= '';
		}
		...
		if(!preg_match('/^[A-Za-z0-9, \'.]+$/i',$doc_symptoms))
		{
			return "symptoms should be character";
		}
		...
		if(isset($_POST['doc_symptoms']))
		{
			...
			foreach(explode(",",$doc_symptoms) as $symptoms)
			{
					...
					VALUES('".$_SESSION['USER_ID']."','".trim($symptoms)."') ");
			}
		}		
